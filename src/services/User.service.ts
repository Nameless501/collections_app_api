...
class UserService {
    ...
    public createUser = (payload: { name: string, email: string, password: string }): Promise<IUserModel> => this.model.create(payload);

    public findUser = async (email: string): Promise<IUserModel> | never => {
        const user: IUserModel | null = await this.model.findOne({ where: { email } });
        if(!user) {
